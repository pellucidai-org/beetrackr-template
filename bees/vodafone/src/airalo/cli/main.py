"""Typer-based CLI for Airalo Scraper.

Run ``airalo --help`` after installing.
"""
...
from airalo.stats import PageStats

app = typer.Typer(
    name="airalo",
    help="Scraper for airalo.com listings.",
...
def _version_callback(value: bool) -> None:
    if value:
        console.print(f"[bold]airalo[/bold] {__version__}")
        raise typer.Exit()
...
def run(
    ...
    if cfg.use_playwright:
        from airalo.scrapers.playwright_runner import run_playwright_target

        written, session = asyncio.run(run_playwright_target(cfg, settings, job_id=jid))
    else:
        from airalo.scrapers.httpx_client import run_httpx_target
...
@app.command("scrape")
def scrape_airalo_cmd(
    limit: Annotated[
        int | None,
        typer.Option(
            "--limit",
            "-n",
            help="Max number of country pages to fetch after the index (smoke tests).",
        ),
    ] = None,
    job_id: Annotated[
        str | None,
    ...
    target: Annotated[
        ...
        typer.Option(
            "--target",
            help="Storage target name (JSONL file / SQL target column).",
        ),
    ] = "airalo",
    show_stats: Annotated[
        bool,
    ...
    """Scrape airalo.com (``/all-esim`` + per-country eSIM pages).

    Dedicated pipeline in ``airalo.scrapers.airalo``. For generic
    config-driven targets, use ``airalo run <target>`` instead.
    """
    from airalo.scrapers.airalo import scrape_airalo
    ...
    log.info("airalo.scrape.starting", job_id=jid, target=target, limit=limit)

    written, session = asyncio.run(
        scrape_airalo(settings, target_name=target, limit=limit, job_id=jid)
    )

    if show_stats:
        _print_session_stats(target, written, session)
...
def db_drop(
    ...
    async def _run() -> None:
        ...
        from airalo.storage.models import Base
